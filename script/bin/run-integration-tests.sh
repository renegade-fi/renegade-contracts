...
show_help() {
    ...
    echo
    echo "Example:"
    echo "  $0                     # Run all tests with hidden Anvil logs"
    echo "  $0 --anvil-logs        # Run all tests with visible Anvil logs"
    echo "  $0 --test test_name    # Run a specific test with hidden Anvil logs"
    echo "  $0 --anvil-logs --test test_name  # Run a specific test with visible Anvil logs"
    echo "  $0 --no-tests          # Start Anvil and deploy contracts without running tests"
    echo "  $0 -r                  # Run all tests in release mode"
}
...
if ! forge script script/DeployDev.s.sol \
    --ffi \
...
if [ "$SKIP_TESTS" = false ]; then
    ...
    if [ -n "$TEST_NAME" ]; then
        ...
        if ! (cd integration && cargo run $CARGO_ARGS -- --test "$TEST_NAME"); then
            echo "Integration tests failed. Cleaning up and exiting..."
            exit 1
        fi
    else
        if ! (cd integration && cargo run $CARGO_ARGS); then
            echo "Integration tests failed. Cleaning up and exiting..."
            exit 1
        fi
    fi
