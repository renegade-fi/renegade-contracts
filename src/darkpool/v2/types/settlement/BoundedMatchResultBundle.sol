...
struct BoundedMatchResultPermit {
    ...
    /// @dev The authorized executor of the bounded match result
    address executor;
}
...
library BoundedMatchResultPermitLib {
    ...
    /// @param self The `BoundedMatchResultPermit` to compute the hash of
    /// @return The hash of the `BoundedMatchResultPermit`
    function computeHash(BoundedMatchResultPermit memory self) internal pure returns (bytes32) {
        return EfficientHashLib.hash(abi.encode(self));
    }
}
