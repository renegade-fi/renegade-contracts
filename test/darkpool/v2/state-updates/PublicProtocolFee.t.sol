...
import { PublicProtocolFeePaymentProofBundle } from "darkpoolv2-types/ProofBundles.sol";
import { ValidPublicProtocolFeePaymentStatement } from "darkpoolv2-lib/public_inputs/Fees.sol";
...
/// @title PublicProtocolFeeTest
/// @author Renegade Eng
/// @notice Tests for the public protocol fee payment functionality in DarkpoolV2
contract PublicProtocolFeeTest is DarkpoolV2TestUtils {
    ...
    /// @notice Create a public protocol fee payment proof bundle
    ...
    function createPublicProtocolFeeProofBundle(
        Note memory note,
        BN254.ScalarField merkleRoot
    )
        internal
        returns (PublicProtocolFeePaymentProofBundle memory proofBundle)
    {
        ...
        ValidPublicProtocolFeePaymentStatement memory statement = ValidPublicProtocolFeePaymentStatement({
    ...
    /// @notice Generate random public protocol fee payment calldata
        ...
        returns (PublicProtocolFeePaymentProofBundle memory proofBundle)
    {
        ...
        proofBundle = createPublicProtocolFeeProofBundle(note, merkleRoot);

        // Capitalize the darkpool to have enough tokens for the fee payment (withdrawal)
        feeToken.mint(address(darkpool), feeAmount);
    }

    /// @notice Generate random public protocol fee payment calldata with a specific amount
    /// @param amount The amount of the fee
    /// @return proofBundle The proof bundle for the fee payment
    ...
    /// @notice Test a successful public protocol fee payment
    function test_payPublicProtocolFee_success() public {
        // Generate test data
        PublicProtocolFeePaymentProofBundle memory proofBundle = generateRandomFeePaymentCalldata();
        ...
        darkpool.payPublicProtocolFee(proofBundle);
    ...
    function test_payPublicProtocolFee_merkleRoot() public {
        // Generate test data
        PublicProtocolFeePaymentProofBundle memory proofBundle = generateRandomFeePaymentCalldata();
    ...
    function test_payPublicProtocolFee_invalidMerkleRoot_reverts() public {
        ...
        PublicProtocolFeePaymentProofBundle memory proofBundle =
            createPublicProtocolFeeProofBundle(note, invalidMerkleRoot);
    ...
    function test_payPublicProtocolFee_doubleSpendNullifier_reverts() public {
        // Generate test data
        PublicProtocolFeePaymentProofBundle memory proofBundle = generateRandomFeePaymentCalldata();
