...
///
/// This computes H(payload) || nonce || chainId
fn compute_payload_with_nonce(payload: &[u8], nonce: U256, chain_id: u64) -> Vec<u8> {
...
impl SignatureWithNonce {
    ...
    pub fn sign(
        ...
        let payload_with_nonce = compute_payload_with_nonce(payload, nonce, chain_id);
        let digest = keccak256(&payload_with_nonce);
    ...
    pub fn validate(
        ...
        let payload_with_nonce = compute_payload_with_nonce(payload, self.nonce, chain_id);
        validate_signature(&payload_with_nonce, &self.signature, expected_address)
    }
}
...
pub fn validate_signature(
    ...
    let signature = Signature::from_bytes_and_parity(&sig[..64], parity);
